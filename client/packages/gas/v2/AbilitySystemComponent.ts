...
import { AbilityQueue, QueueOptions, QueueExecutionMode } from './AbilityQueue';
import { 
  AbilityContext, 
...
export class EnhancedAbilitySystemComponent {
  ...
  constructor(owner: any) {
    this.owner = owner;
    this.abilityQueue = new AbilityQueue(this);
    this.setupDefaultEventHandlers();
    this.emit('asc-initialized', { owner, timestamp: Date.now() });
  }
  ...
  queueAbility(
    ...
    return this.abilityQueue.enqueue(abilityId, context, options);
  }

  cancelQueuedAbility(queueId: string): boolean {
    return this.abilityQueue.cancel(queueId);
  }

  clearAbilityQueue(): number {
    return this.abilityQueue.cancelAll();
  }

  getQueueSize(): number {
    return this.abilityQueue.getSize();
  }

  getQueueStats() {
    return this.abilityQueue.getStats();
  }

  setQueueMode(mode: QueueExecutionMode): void {
    this.abilityQueue.setMode(mode);
  }

  getQueueMode(): QueueExecutionMode {
    return this.abilityQueue.getMode();
  }

  processAbilityQueue(): Promise<number> {
    return this.abilityQueue.execute();
  }
  ...
  update(deltaTime: number): void {
    ...
    this.abilityQueue.processQueue();
  }
  ...
  destroy(): void {
    this.emit('asc-destroyed', { owner: this.owner, timestamp: Date.now() });
    this.abilityQueue.destroy();
