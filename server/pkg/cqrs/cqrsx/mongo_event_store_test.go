...
func TestMongoEventStore_BasicOperations(t *testing.T) {
	...
	t.Run("SaveMultipleEventsSequentially", func(t *testing.T) {
		...
		// Save first event
		event1 := cqrs.NewBaseEventMessage("Event1", aggregateID2, aggregateType, 1, map[string]interface{}{"data": "data1"})
		err := eventStore.SaveEvents(ctx, aggregateID2, []cqrs.EventMessage{event1}, 0)
		assert.NoError(t, err)

		// Save second event
		event2 := cqrs.NewBaseEventMessage("Event2", aggregateID2, aggregateType, 2, map[string]interface{}{"data": "data2"})
		err = eventStore.SaveEvents(ctx, aggregateID2, []cqrs.EventMessage{event2}, 1)
		assert.NoError(t, err)

		// Save third event
		event3 := cqrs.NewBaseEventMessage("Event3", aggregateID2, aggregateType, 3, map[string]interface{}{"data": "data3"})
	...
	t.Run("GetLastEventVersion", func(t *testing.T) {
		...
		// Save one event
		event1 := cqrs.NewBaseEventMessage("Event1", aggregateID3, aggregateType, 1, map[string]interface{}{"data": "data1"})
		...
		// Save another event
		event2 := cqrs.NewBaseEventMessage("Event2", aggregateID3, aggregateType, 2, map[string]interface{}{"data": "data2"})
	...
	t.Run("ConcurrencyControl", func(t *testing.T) {
		...
		// Save first event
		event1 := cqrs.NewBaseEventMessage("Event1", aggregateID4, aggregateType, 1, map[string]interface{}{"data": "data1"})
		err := eventStore.SaveEvents(ctx, aggregateID4, []cqrs.EventMessage{event1}, 0)
		assert.NoError(t, err)

		// Try to save with wrong expected version - should fail
		event2 := cqrs.NewBaseEventMessage("Event2", aggregateID4, aggregateType, 2, map[string]interface{}{"data": "data2"})
	...
	t.Run("LoadEventsFromVersion", func(t *testing.T) {
		...
		events := []cqrs.EventMessage{
			cqrs.NewBaseEventMessage("Event1", aggregateID5, aggregateType, 1, map[string]interface{}{"data": "data1"}),
			cqrs.NewBaseEventMessage("Event2", aggregateID5, aggregateType, 2, map[string]interface{}{"data": "data2"}),
			cqrs.NewBaseEventMessage("Event3", aggregateID5, aggregateType, 3, map[string]interface{}{"data": "data3"}),
		}
...
func TestMongoEventStore_EdgeCases(t *testing.T) {
	...
	t.Run("EmptyAggregateID", func(t *testing.T) {
		event := cqrs.NewBaseEventMessage("TestEvent", "", "TestAggregate", 1, map[string]interface{}{"data": "test"})
